...
public class StatusBarTool {
    ...
    private static volatile StatusBarTool instance;// !!必须要加volatile限制指令重排序，不然这是双重检验的漏洞
    ...
    //单例模式，懒汉氏
    public static StatusBarTool instance() {
        if (instance == null) {
            synchronized (lock) {
                if (instance == null) {
                    instance = new StatusBarTool();
                }
            }
        }
        return instance;
    ...
    public  final int DEFAULT_STATUS_BAR_ALPHA = 255;
    private  final int FAKE_STATUS_BAR_VIEW_ID = R.id.statusbarutil_fake_status_bar_view;
    private  final int FAKE_TRANSLUCENT_VIEW_ID = R.id.statusbarutil_translucent_view;
    private  final int TAG_KEY_HAVE_SET_OFFSET = -123;
...
//        if (Build.MODEL.equals("CLT-AL00") || Build.MODEL.equals("CLT-AL01")) {
//            setColor(activity,activity.getResources().getColor(R.color.black), 0);
//            return;
//        } else {
...
//        }
     ...
     * @param isShow         是否下移
     */
    public void setTranslucentForView(Activity activity, View needOffsetView, boolean isShow) {
        setTranslucentForImageView(activity, 0, needOffsetView, isShow);
    }
     ...
     */
    public void setTranslucentForImageView(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
                                                  View needOffsetView, boolean isShow) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
        ...
        if (needOffsetView != null) {
            ...
            ViewGroup.MarginLayoutParams layoutParams = (ViewGroup.MarginLayoutParams) needOffsetView.getLayoutParams();
            if (haveSetOffset != null && (Boolean) haveSetOffset) {
                needOffsetView.setTag(TAG_KEY_HAVE_SET_OFFSET, isShow);
                if (isShow) {
                    return;
                } else {
            ...
            int top = isShow ? layoutParams.topMargin + getStatusBarHeight(activity) : layoutParams.topMargin;
...
//                View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN |
    ...
    public void fullScreen(Activity activity) {
        if (Build.MODEL.equals("CLT-AL00") || Build.MODEL.equals("CLT-AL01")) {    //华为p20适配
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
